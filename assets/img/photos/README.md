...
| `workings.jpg` | Photographs — the mine (3:4) | A cut rock face in the workings, water standing at its foot |
...
`gallery.html` shows twenty-one of these photographs in a masonry layout, grouped as the
mine, the works, power and environment, and recognition. It reuses the published files
rather than holding its own copies — the only frame added for it was `workings.jpg`.
